...
SELECT EVENT_NUMBER FROM EVENTS WHERE RAID_POINTS+DEFENDING_POINTS>=3 ORDER BY EVENT_NO ASC;
...
-- 9. **Write a query to display the winner team ids and the scores of those teams whose scores
--are less than 35. Display the records sorted in descending order based on outcome id.**
SELECT WINNER_TEAM_ID,SCORE FROM OUTCOME WHERE SCORE <35 ORDER BY ID DESC;
-- 10. **Write a query to display the game details of those games that were played between
--January and March of 2020. Display the records sorted in descending order based on id.**
SELECT * FROM GAME WHERE GAME_DATE BETWEEN '01/JAN/2020' AND '01/MAR/2020' ORDER BY ID DESC;

-- 11. **Write a query to display the names of players who are Raiders. Display the records
--sorted in ascending order based on player name.**
SELECT NAME FROM PLAYER WHERE NAME='RAIDER' ORDER BY NAME ASC;

-- 12. **Write a query to display the event number, raid points and defence
--points scored in the first innings of the game that was played on `2020-01-26`. 
--Display the records sorted in ascending order based on event number.**
SELECT EVENT_NO,RAID_POINTS,DEFENDING_POINTS FROM EVENT WHERE INNINGS_ID 
IN(SELECT FIRST_INNINGS_ID FROM GAME WHERE GAME_DATE='26/JAN/2020') ORDER BY EVENT_NO ASC;
...
SELECT STADIUM_NAME FROM VENUE,GAME WHERE GAME_DATE='09/MAR/2020' ORDER BY STADIUM_NAME ASC;
